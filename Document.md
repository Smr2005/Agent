...
### Key Features:
- ✅ Admin-controlled user registration with temporary passwords
- ✅ One project per student constraint (enforced at database level)
- ✅ AI-powered message sanitization using Anthropic Claude API
- ✅ WhatsApp-style color-coded messaging interface
- ✅ File sharing with automatic format detection (50MB per file limit)
- ✅ Automatic contact information filtering from messages
...
└── static/
    └── js/
        ...
        └── developer.js        # Developer interface
```
...
│   ├── message.py         # Project messages (agent routing)
│   ├── direct_message.py  # Direct messages
...
│   ├── messaging.py      # Messaging & file upload
│   ├── queue.py          # Project queue management
│   └── seed.py           # Database seeding
├── services/             # Business logic
│   ├── auth.py          # JWT creation/validation
│   ├── ai_agent.py      # AI message processing
│   └── assignment.py    # Project assignment logic
...
- When student sends message: Creates 2 messages
  1. `student → agent`
  2. `agent → developer` (AI-sanitized)
- When developer sends message: Creates 2 messages
  1. `developer → agent`
  2. `agent → student` (AI-sanitized)
...
**Purpose:** Store direct messages between users (e.g., student ↔ admin)
...
Filter contact info
    ↓
Create message: developer → agent
    ↓
AI Agent processes via Claude API
    ↓
Claude converts to student-friendly language
    ↓
Create message: agent → student (sanitized text)
    ↓
Both messages stored in database
```
...
```
Email: name@domain.com        → [EMAIL REMOVED]
Phone: +1-234-567-8900       → [PHONE REMOVED]
Social: instagram.com/user   → [SOCIAL MEDIA REMOVED]
WhatsApp: wa.me/phonenumber  → [SOCIAL MEDIA REMOVED]
```
...
- `student → agent` (original student message)
- `agent → developer` (sanitized for developer)
- `developer → agent` (original developer message)
- `agent → student` (sanitized for student)
...
2. Add new extension → MIME type mapping
3. Restart backend
...
4. Backend validates: Student has no project ✅
...
9. Backend:
   ...
   - Finds agent→developer message
   - Attaches file to BOTH messages

10. Developer logs in → sees project
11. Enters project chat → sees file
12. Clicks download → file downloads as product-list.csv
```
...
3. Creates messages:
   - developer → agent (original)
   - agent → student (sanitized)
...
5. Student logs in → sees project chat
...
5. Backend checks:
   ...
   - File exists? ✅
   - Look up MIME: .pptx → application/vnd.openxmlformats-officedocument.presentationml.presentation
...
**Solution:** File must be attached to BOTH messages (student→agent AND agent→developer) for cross-visibility.
